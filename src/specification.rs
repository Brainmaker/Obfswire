...
// ```text
// | salt | reserved | direction | stream_id | timestamp | body_len | payload_len | tag |     ...    |
// |  32B |    1B    |     1B    |     8B    |     8B    |    2B    |     2B      | 16B |            |
// |                                    <- header ->                                    | <- body -> |
// |                                          <- init_frame ->                                       |
//
// |        ...         | payload  | padding  | tag |
// |                    | variable | variable | 16B |
// |    <- header ->    |        <- body ->         |
// |                 <- init_frame ->               |
// ```
...
pub(crate) const INIT_HDR_LEN: usize = 32 + 1 + 1 + 8 + 8 + 2 + 2 + 16; // 70
...
// Stream frame:
// ```text
// | command | body_len | payload_len | tag | payload  | padding  | tag |
// |    1B   |    2B    |     2B      | 16B | variable | variable | 16B |
// |              <- header ->              |         <- body ->        |
// |                           <- frame ->                              |
//
// ```
...
pub(crate) const HDR_LEN: usize = 1 + 2 + 2 + 16; // 21
